...
public class CredentialsRepository {
    ...
    public Optional<ZappiCredentials> read(String userId) {
        ...
        return Optional.of(new ZappiCredentials(userId, serialNumber.getS(), code, encryptedApiKey));
    }

    public void write(ZappiCredentials creds) {
        var item = new HashMap<String, AttributeValue>();
        item.put(USER_ID_COLUMN, new AttributeValue(creds.getUserId()));
        item.put(CODE_COLUMN, new AttributeValue(creds.getCode()));
        item.put(SERIAL_NUMBER_COLUMN, new AttributeValue(creds.getSerialNumber()));
