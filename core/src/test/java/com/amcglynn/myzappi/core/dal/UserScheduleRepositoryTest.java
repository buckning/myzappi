...
class UserScheduleRepositoryTest {
    ...
    void testReadForUserWhoHasEntryInDb() {
        when(mockDb.getItem(any(GetItemRequest.class))).thenReturn(GetItemResponse.builder()
                .item(Map.of("user-id", stringValue("testuser"),
                        "schedules", stringValue(testScheduleString)))
                .build());
        var result = repository.read(UserId.from("userid"));
    ...
    void testReadForUserWhoHasEntryInDbWithScheduleActionTarget() {
        when(mockDb.getItem(any(GetItemRequest.class))).thenReturn(GetItemResponse.builder()
                .item(Map.of("user-id", stringValue("testuser"),
                        "schedules", stringValue(testScheduleWithTargetString)))
                .build());
        var result = repository.read(UserId.from("userid"));
        assertThat(result).hasSize(1);
    ...
    void testReadForUserWhoHasEntryInDbWithDisabledSchedule() {
        when(mockDb.getItem(any(GetItemRequest.class))).thenReturn(GetItemResponse.builder()
                .item(Map.of("user-id", stringValue("testuser"),
                        "schedules", stringValue(testDisabledScheduleString)))
                .build());
        var result = repository.read(UserId.from("userid"));
        assertThat(result).hasSize(1);
    ...
    void testWrite() {
        ...
        assertThat(putItemCaptor.getValue().tableName()).isEqualTo("schedule");
        assertThat(putItemCaptor.getValue().item().get("user-id").s()).isEqualTo("testUser");
        assertThat(putItemCaptor.getValue().item().get("schedules").s())
    ...
    void testWriteRecurringSchedule() throws Exception {
        ...
        assertThat(putItemCaptor.getValue().tableName()).isEqualTo("schedule");
        assertThat(putItemCaptor.getValue().item().get("user-id").s()).isEqualTo("testUser");
        var schedulesFromDb = new ObjectMapper().readValue(putItemCaptor.getValue().item().get("schedules").s(), new TypeReference<List<Schedule>>() {
        });
    ...
    void testUpdate() {
        ...
        assertThat(updateItemCaptor.getValue().tableName()).isEqualTo("schedule");
        assertThat(updateItemCaptor.getValue().updateExpression()).isEqualTo("SET #schedules = :schedules");
        assertThat(updateItemCaptor.getValue().expressionAttributeValues().get(":schedules").s())
                .isEqualTo("[{\"id\":\"1234567890\",\"startDateTime\":\"2023-09-08T14:00\"," +
                        "\"zoneId\":\"Europe/Dublin\",\"action\":{\"type\":\"chargeMode\",\"value\":\"ECO+\"}}]");
        assertThat(updateItemCaptor.getValue().key().get("user-id").s()).isEqualTo("testUser");
    }
}
